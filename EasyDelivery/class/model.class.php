	...
	public function setDriverPosition()
   	{
		try{			
			...
			$query_string = $this->con->prepare("INSERT INTO ed_driverposition (id, CompanyID, DeliveryID, LineID, Latitude, Longitude, CreationDate) 
												  VALUES (:id, :companyid, :deliveryid, :lineid, :latitude, :longitude, CURRENT_TIMESTAMP)");
		...
		try {
			...
			if ($ms) {
				while ($ms) {
					...
					$deliveryline["deliveryid"] = $ms["ID"];
					$deliveryline["companyid"] = $ms["CompanyID"];
					$deliveryline["description"] = $ms["Description"];
					$deliveryline["driverid"] = ($ms["DriverID"]=='')?'0':$ms["DriverID"];
					$deliveryline["vehicleid"] = ($ms["VehicleID"]=='')?'0':$ms["VehicleID"];
					$deliveryline["datecreated"] = $ms["DateCreated"];
					$deliveryline["datecompleted"] = $ms["DateCompleted"];
					$deliveryline["isdriverbeginning"] = $ms["isDriverBeginning"];
					$deliveryline["dateinitiated"] = $ms["DateInitiated"];
					$deliveryline["lineid"] = $ms["LineID"];
					$deliveryline["customerid"] = $ms["CustomerID"];
					$deliveryline["linelatitude"] = $ms["LineLatitude"];
					$deliveryline["linelongitude"] = $ms["LineLongitude"];
					
					// deliveryline node
					$ret["deliveryline"] = array();
            		array_push($ret["deliveryline"], $deliveryline);
